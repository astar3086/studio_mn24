...
class Command
{
	...
	protected function queryInternal($method,$mode,$params=[])
	{
		...
		try
		{
			...
			{
				//var_echo('system.db.CDbCommand.query('.$this->getText().$par.')','system.db.CDbCommand.query');
			}
	...
	public function execute($params=[])
	{
		...
		try
		{
			if($this->_connection->enableProfiling)
			{
				//var_echo('system.db.CDbCommand.execute('.$this->getText().$par.')','system.db.CDbCommand.execute');
			}
	...
	public function andWhere($conditions,$params=[])
	{
		if(isset($this->_query['where']))
			$this->_query['where']=$this->processConditions(['AND',$this->_query['where'],$conditions]);
		else
			$this->_query['where']=$this->processConditions($conditions);
	...
	{
		if(isset($this->_query['where']))
			$this->_query['where']=$this->processConditions(['OR',$this->_query['where'],$conditions]);
		else
			$this->_query['where']=$this->processConditions($conditions);
