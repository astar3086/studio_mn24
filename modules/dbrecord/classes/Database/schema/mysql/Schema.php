...
class Schema extends BaseSchema\Schema
{
	 ...
	 * @param string $name table name
	 * @return BaseSchema\TableSchema driver dependent table metadata. Null if the table does not exist.
	 */
	 ...
	 * @param \Database\schema\mysql\TableSchema $table the table metadata
	 ...
	 * @param BaseSchema\mysql\TableSchema $table the table metadata
	 */
	protected function findConstraints($table)
	{
		...
		foreach($matches as $match)
		{
			...
			{
				$table->foreignKeys[$name]=[str_replace(['`','"'],'',$match[2]),$fks[$k]];
	 ...
	 * @return \Database\schema\CommandBuilder command builder instance
	 * @since 1.1.13
	 */
	 ...
	 * @throws \Database\Exception if specified column is not found in given table
	...
	public function renameColumn($table, $name, $newName)
	{
		...
		if($row===false)
			throw new \Database\Exception(Yii::t('yii','Unable to find "{column}" in table "{table}".',['{column}'=>$name,'{table}'=>$table]));
