<?php
/**
 * \Database\ActiveRecord\Record class file.
 *
...
/**
 * \Database\ActiveRecord\Record is the base class for classes representing relational data.
 *
 ...
 * about this class.
 *
 * @property \Database\schema\Criteria $dbCriteria The query criteria that is associated with this model.
 * This criteria is mainly used by {@link scopes named scope} feature to accumulate
 * different criteria specifications.
 * @property \Database\ActiveRecord\MetaData $metaData The meta for this AR class.
 * @property \Database\Connection $dbConnection The database connection used by active record.
 * @property \Database\schema\TableSchema $tableSchema The metadata of the table that this AR belongs to.
 * @property \Database\schema\CommandBuilder $commandBuilder The command builder used by this AR.
...
//use Database\ActiveRecord\MetaData;
use Model;
...
abstract class Record extends Model
{
	 ...
	 *
	 */
	const BELONGS_TO='\Database\ActiveRecord\Relationship\BelongsTo';
	/**
	 *
	 */
	const HAS_ONE='\Database\ActiveRecord\Relationship\HasOne';
	/**
	 *
	 */
	const HAS_MANY='\Database\ActiveRecord\Relationship\HasMany';
	/**
	 *
	 */
	const MANY_MANY='\Database\ActiveRecord\Relationship\ManyMany';
	/**
	 *
	 */
	const STAT='\Database\ActiveRecord\Relationship\Stat';

	/**
	 * @var \Database\Connection the default database connection for all active record classes.
	 ...
	 * @var array
	 */
	private static $_md=[];				// class name => meta data
    ...
    /**
     * Helps to avoid "Object could not be converted to string" error
     *
     * @return string class name + primary key
     * for PHP < 5.3 used "__CLASS__" value instead of the "Late Static Binding" class name
     */
    public function __toString()
    {
        return function_exists('get_called_class')
            ? get_called_class().'#'.(string)$this->getPrimaryKey()
            : __CLASS__.'#'.(string)$this->getPrimaryKey();
    }
	...
	public function __construct($scenario='insert')
	{
		...
		$this->init();
		//#TODO Закоментировано. Не извесное поведение.
    ...
    public static function getList($name = "name", $condition = '', $params=[], $additional = array())
    {
        $list = [];

        $entities = self::model()->findAll($condition, $params);
        ...
        if ( empty($additional) ){
            ...
            {
                $list[$entity->getPrimaryKey()] = $entity->$name;
            }
        } else {
            foreach($entities as $entity)
            {
                ...
                $list[$entity->getPrimaryKey()] = implode( '##', $columns );
            }
        }
	 ...
	 * This is a shortcut method to {@link \Database\Connection::cache()}.
	 ...
	 * @return Record the active record instance itself.
	 * @since 1.1.7
	 */
	...
	public function __get($name)
	{
		...
		{
			throw new \Database\Exception('Artibute '.$name.' is not found');
			/*var_dump($this->getMetaData()->relations);
			var_dump($this->getMetaData()->relations[$name]);
			var_dump($name);*/
			//return false;
		}
	}
	 ...
	 * @param mixed $params    array or \Database\schema\Criteria object with additional parameters that customize the query conditions as specified in the relation declaration.
	...
	public function getRelated($name,$refresh=false,$params=[])
	{
		...
		if($this->getIsNewRecord() && !$refresh && ($relation instanceof Relationship\HasOne || $relation instanceof Relationship\HasMany))
			return $relation instanceof Relationship\HasOne ? null : [];

		if($params!==[]) // dynamic query
		{
			/** @var bool $exists bool */
			$exists=isset($this->_related[$name]) || array_key_exists($name,$this->_related);
			if($exists)
				$save=$this->_related[$name];

			if($params instanceof Criteria)
		...
		{
			if($relation instanceof Relationship\HasMany)
				$this->_related[$name]=[];
			elseif($relation instanceof Relationship\Stat)
	 ...
	 * of {@link \Database\schema\Criteria}.
	 */
	 ...
	 * @return Record active record model instance.
	 */
	 ...
	 * @return MetaData the meta for this AR class.
	 */
	public function getMetaData()
	{
		...
		if(!array_key_exists($className,self::$_md))
		{
			...
			/** @var \Database\ActiveRecord\Record $this */
			self::$_md[$className]= new MetaData($this);
		}
	 ...
	 * available table schema. Make sure you have called {@link \Database\schema\Schema::refresh}
	 ...
	 * as an array whose keys must be properties of {@link \Database\schema\Criteria}.
	 */
	 ...
	 * @throws Exception if "db" application component is not defined
	 * @return \Database\Connection the database connection used by active record.
	 */
	public function getDbConnection()
	{
		...
		{
			#@TODO Import Kohana::$config
			$DB_conf = \Kohana::$config->load('database')->get('default');
			self::$db= new Connection($DB_conf);
			self::$db->init();
			if(self::$db instanceof Connection)
				return self::$db;
			else
				throw new Exception('Active Record requires a "db" \Database\Connection application component.');
		}
	}
	 ...
	 * @param string $name the relation name
	 * @return \Database\ActiveRecord\Relationship\Active the named relation declared for this AR class. Null if the relation does not exist.
	 */
	 ...
	 * @return \Database\schema\TableSchema the metadata of the table that this AR belongs to
	 */
	 ...
	 * @return \Database\schema\CommandBuilder the command builder used by this AR
	 */
	 ...
	 * @return string the attribute value. Null if the attribute is not set or does not exist.
	 * @see hasAttribute
	 */
	...
	/**
	 * Do not call this method. This method is used internally by {@link \Database\ActiveRecord\Finder} to populate
	 ...
	 * @throws Exception
	 * @internal param \Database\ActiveRecord\CModelEvent $event the event parameter
	 */
	 ...
	 * @throws Exception
	 * @internal param \Database\ActiveRecord\CEvent $event the event parameter
	 */
	 ...
	 * This can be either a call to \Database\ActiveRecord\Records find methods or a find call
	 ...
	 * @return \Database\ActiveRecord\Finder active finder for the operation
	 *
	 * @since 1.1.14
	 */
	public function getActiveFinder($with)
	{
		return new Finder($this,$with);
	}
	...
	{
		#@TODO hasEventHandler onAfterDelete
		throw new Exception('hasEventHandler: onBeforeCount dont realised!');
	...
	{
		//throw new Exception('hasEventHandler : onBeforeSave dose not realised');
	...
	{
		#@TODO hasEventHandler onAfterSave
		//throw new Exception('hasEventHandler: onAfterSave dont realised!');
	...
	{
		#@TODO hasEventHandler onAfterDelete
		//throw new Exception('hasEventHandler: onAfterDelete dont realised!');
	...
	{
		#@TODO hasEventHandler onBeforeFind
		//throw new Exception('hasEventHandler: onBeforeFind dont realised!');
	...
	{
		#@TODO hasEventHandler onBeforeCount
		//throw new Exception('hasEventHandler: onBeforeCount dont realised!');
	...
	{
		#@TODO hasEventHandler: onAfterFind dont realised!
		//throw new Exception('hasEventHandler: onAfterFind dont realised!');
	...
	public function insert($attributes=null)
	{
		...
		if($this->beforeSave())
		{
			//Yii::trace(get_class($this).'.insert()','system.db.ar.\Database\ActiveRecord\Record');
	...
	public function update($attributes=null)
	{
		...
		if($this->beforeSave())
		{
			//Yii::trace(get_class($this).'.update()','system.db.ar.\Database\ActiveRecord\Record');
			if($this->_pk===null)
				$this->_pk=$this->getPrimaryKey();
	...
	public function saveAttributes($attributes)
	{
		if(!$this->getIsNewRecord())
		{
			//Yii::trace(get_class($this).'.saveAttributes()','system.db.ar.\Database\ActiveRecord\Record');
			$values=[];
	...
	{
		//Yii::trace(get_class($this).'.saveCounters()','system.db.ar.\Database\ActiveRecord\Record');
		$builder=$this->getCommandBuilder();
	...
	public function delete()
	{
		if(!$this->getIsNewRecord())
		{
			//Yii::trace(get_class($this).'.delete()','system.db.ar.\Database\ActiveRecord\Record');
	...
	{
		//Yii::trace(get_class($this).'.refresh()','system.db.ar.\Database\ActiveRecord\Record');
	 ...
	 * If an array, it is treated as the initial values for constructing a {@link \Database\schema\Criteria} object;
	 * Otherwise, it should be an instance of {@link \Database\schema\Criteria}.
	 * @param array $params parameters to be bound to an SQL statement.
	 * This is only used when the first parameter is a string (query condition).
	 * In other cases, please use {@link \Database\schema\Criteria::params} to set parameters.
	 * @return Record the record found. Null if no record is found.
	 */
	public function find($condition='',$params=[])
	{
		//Yii::trace(get_class($this).'.find()','system.db.ar.\Database\ActiveRecord\Record');
	 ...
	 * @param mixed $condition query condition or criteria.
	 * @param array $params parameters to be bound to an SQL statement.
	 * @return \Database\ActiveRecord\Record[] list of active records satisfying the specified condition. An empty array is returned if none is found.
	 */
	public function findAll($condition='',$params=[])
	{
		//Yii::trace(get_class($this).'.findAll()','system.db.ar.\Database\ActiveRecord\Record');
		$criteria=$this->getCommandBuilder()->createCriteria($condition,$params);
	 ...
	 * @param mixed $condition query condition or criteria.
	 * @param array $params parameters to be bound to an SQL statement.
	 * @return Record the record found. Null if none is found.
	 */
	public function findByPk($pk,$condition='',$params=[])
	{
		//Yii::trace(get_class($this).'.findByPk()','system.db.ar.\Database\ActiveRecord\Record');
	 ...
	 * @param mixed $condition query condition or criteria.
	 * @param array $params parameters to be bound to an SQL statement.
	 * @return Record[] the records found. An empty array is returned if none is found.
	 */
	public function findAllByPk($pk,$condition='',$params=[])
	{
		//Yii::trace(get_class($this).'.findAllByPk()','system.db.ar.\Database\ActiveRecord\Record');
		$prefix=$this->getTableAlias(true).'.';
		$criteria=$this->getCommandBuilder()->createPkCriteria($this->getTableSchema(),$pk,$condition,$params,$prefix);
		return $this->query($criteria,true);
	}
	...
	{
		//Yii::trace(get_class($this).'.findByAttributes()','system.db.ar.\Database\ActiveRecord\Record');
		$prefix=$this->getTableAlias(true).'.';
	 ...
	 * @param mixed $condition query condition or criteria.
	 * @param array $params parameters to be bound to an SQL statement.
	 * @return \Database\ActiveRecord\Record[] the records found. An empty array is returned if none is found.
	 */
	public function findAllByAttributes($attributes,$condition='',$params=[])
	{
		//Yii::trace(get_class($this).'.findAllByAttributes()','system.db.ar.\Database\ActiveRecord\Record');
		$prefix=$this->getTableAlias(true).'.';
		$criteria=$this->getCommandBuilder()->createColumnCriteria($this->getTableSchema(),$attributes,$condition,$params,$prefix);
		return $this->query($criteria,true);
	}
	...
	{
		//Yii::trace(get_class($this).'.findBySql()','system.db.ar.\Database\ActiveRecord\Record');
		$this->beforeFind();
	...
	{
		//Yii::trace(get_class($this).'.findAllBySql()','system.db.ar.\Database\ActiveRecord\Record');
		$this->beforeFind();
		if(($criteria=$this->getDbCriteria(false))!==null && !empty($criteria->with))
		{
			$this->resetScope(false);
	...
	{
		//Yii::trace(get_class($this).'.count()','system.db.ar.\Database\ActiveRecord\Record');
		$builder=$this->getCommandBuilder();
		$this->beforeCount();
	...
	{
		//Yii::trace(get_class($this).'.countByAttributes()','system.db.ar.\Database\ActiveRecord\Record');
		$prefix=$this->getTableAlias(true).'.';
		$builder=$this->getCommandBuilder();
		$this->beforeCount();
	 ...
	 * This is equivalent to calling {@link \Database\Command::queryScalar} with the specified
	...
	{
		//Yii::trace(get_class($this).'.countBySql()','system.db.ar.\Database\ActiveRecord\Record');
		$this->beforeCount();
	...
	{
		//Yii::trace(get_class($this).'.exists()','system.db.ar.\Database\ActiveRecord\Record');
		$builder=$this->getCommandBuilder();
		$criteria=$builder->createCriteria($condition,$params);
		$table=$this->getTableSchema();
		$criteria->select='1';
	 ...
	 * @return Record the AR object itself.
	 */
	...
	/**
	 * Sets {@link \Database\schema\Criteria::together} property to be true.
	 * This is only used in relational AR query. Please refer to {@link \Database\schema\Criteria::together}
	 * for more details.
	 * @return Record the AR object itself
	 * @since 1.1.4
	 */
	...
	{
		//Yii::trace(get_class($this).'.updateByPk()','system.db.ar.\Database\ActiveRecord\Record');
		$builder=$this->getCommandBuilder();
		$table=$this->getTableSchema();
	...
	{
		//Yii::trace(get_class($this).'.updateAll()','system.db.ar.\Database\ActiveRecord\Record');
		$builder = $this->getCommandBuilder();
		$criteria = $builder->createCriteria($condition,$params);
	...
	{
		//Yii::trace(get_class($this).'.updateCounters()','system.db.ar.\Database\ActiveRecord\Record');
		$builder=$this->getCommandBuilder();
		$criteria=$builder->createCriteria($condition,$params);
		$command=$builder->createUpdateCounterCommand($this->getTableSchema(),$counters,$criteria);
		return $command->execute();
	...
	{
		//Yii::trace(get_class($this).'.deleteByPk()','system.db.ar.\Database\ActiveRecord\Record');
		$builder=$this->getCommandBuilder();
	...
	{
		//Yii::trace(get_class($this).'.deleteAll()','system.db.ar.\Database\ActiveRecord\Record');
		$builder=$this->getCommandBuilder();
		$criteria=$builder->createCriteria($condition,$params);
		$command=$builder->createDeleteCommand($this->getTableSchema(),$criteria);
		return $command->execute();
	...
	{
		//Yii::trace(get_class($this).'.deleteAllByAttributes()','system.db.ar.\Database\ActiveRecord\Record');
		$builder=$this->getCommandBuilder();
		$table=$this->getTableSchema();
	 ...
	 * @return Record the newly created active record. The class of the object is the same as the model class.
	 ...
	 * @internal param array $attributes list of attribute values for the active records.
	 * @internal param $attributes
	 * @return Record the active record
	 */
