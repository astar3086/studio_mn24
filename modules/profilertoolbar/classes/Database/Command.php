<?php
/**
 * This file contains the \Database\Command class.
...
/**
 * \Database\Command represents an SQL statement to execute against a database.
 * It is usually created by calling {@link \Database\Connection::createCommand}.
 ...
 * \Database\Command supports SQL statement preparation and parameter binding.
 ...
 * Starting from version 1.1.6, \Database\Command can also be used as a query builder
 ...
 * @property \Database\Connection $connection The connection associated with this command.
 ...
 * @package Database
 */
/** [FIX] Multiple Define in PHPStorm */
...
class Command
{
	...
	public function execute($params = [])
	{
		...
		//var_echo('Executing SQL: ' . $this->getText() . $par, 'system.db.CDbCommand');
		try
		{
			...
			if($this->_connection->enableProfiling)
			{
				...
				\ProfilerToolbar::setSqlData('default', $this->Debug($this->getText(),$this->_paramLog), sizeof($n));
			}
			return $n;
		}
		catch(Exception $e)
	...
	private function queryInternal($method, $mode, $params = [])
	{
		...
		try
		{
			...
			{
				\Cache::instance('database')
					->set($this->getText(), $result, $this->_connection->caching);
			}
	 ...
	 * @return string
	 */
	public function Debug($statement, $dataArray = []){
	 ...
	 * @return \Database\Command this command instance
	 * @since 1.1.6
	 */
	 ...
	 * @throws \Database\Exception execution failed
	 */
	...
	public function buildQuery($query)
	{
		...
		if($limit >= 0 || $offset > 0)
		{
			$sql = $this->_connection->getCommandBuilder()
	 ...
	 * @return \Database\Command the command object itself
	 * @since 1.1.6
	 */
	...
	public function andWhere($conditions, $params = [])
	{
		...
		{
			$this->_query['where'] = $this->processConditions([
				'AND',
				$this->_query['where'],
				$conditions
			]);
	...
	public function truncateTable($table)
	{
		$schema = $this->getConnection()
			->getSchema();
		$n = $this->setText($schema->truncateTable($table))
			->execute();
		if(strncasecmp($this->getConnection()
				->getDriverName(), 'sqlite', 6) === 0
		)
		{
