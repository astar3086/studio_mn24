<div id="ptb_data_cont_vars" class="ptb_data_cont" style="display: none;">
	<ul class="ptb_tabs">
		<li id="ptb_tab_varsPGF">$_POST<span>(<?php echo count(ProfilerToolbar::$DATA_POST); ?>)</span> /
			$_GET<span>(<?php echo count(ProfilerToolbar::$DATA_GET); ?>)</span> /
			$_FILES<span>(<?php echo count(ProfilerToolbar::$DATA_FILES); ?>)</span></li>
		<li id="ptb_tab_varsCS">$_COOKIE<span>(<?php echo count(ProfilerToolbar::$DATA_COOKIE); ?>)</span> /
			$_SESSION<span>(<?php echo count(ProfilerToolbar::$DATA_SESSION); ?>)</span></li>
		<li id="ptb_tab_varsS">$_SERVER</li>
	</ul>
	<div class="ptb_tab_cont" id="ptb_tab_cont_varsPGF">
		<!-- POST/GET/FILES -->
		<?php $vars = array(
			'POST' => ProfilerToolbar::$DATA_POST,
			'GET'  => ProfilerToolbar::$DATA_GET
		); ?>
		...
		<table class="centr" style="clear: both;">
			...
			<tbody>
			<?php if(empty(ProfilerToolbar::$DATA_FILES)): ?>
				<tr>
					<td colspan="7" class="empty">—</td>
				</tr><?php else: ?>
				<?php $i = 0;
				foreach(ProfilerToolbar::$DATA_FILES as $k => $v): ?>
					<tr>
						<td class="num"><?php echo ++$i; ?></td>
	...
	<div class="ptb_tab_cont" id="ptb_tab_cont_varsCS">
		<!-- COOKIE/SESSION -->
		<?php $vars = array(
			'COOKIE'  => ProfilerToolbar::$DATA_COOKIE,
			'SESSION' => ProfilerToolbar::$DATA_SESSION
		); ?>
		<?php foreach($vars as $kvar => $var): ?>
