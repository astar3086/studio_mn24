					...
					<?php foreach($group['data'] as $i => $v): ?>
						<tr>
							...
							<td>
								...
								<?php endif; ?>
								<?php if($highlightSQL): ?>
									<pre class="source sql"><?php echo ProfilerToolbar::highlight($v['sql'],
											'sql'); ?></pre>
								<?php else: ?>
									<?php echo $v['sql']; ?>
								<?php endif; ?>
							...
							<td class="tRight graph">
								...
								<div class="line"
								     style="width:<?php echo round($v['time'] / $group['total']['time'] * 100); ?>%;"></div>
							</td>
							<td class="tRight graph">
								<div class="val"><?php echo ProfilerToolbar::formatMemory($v['memory']); ?></div>
								<div class="line"
								     style="width:<?php echo round($v['memory'] / $group['total']['memory'] * 100); ?>%;"></div>
							</td>
						</tr>
					<?php endforeach; ?>
					<tr class="total">
