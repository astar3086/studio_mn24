<div id="ptb_data_cont_files" class="ptb_data_cont" style="display: none;">
	<ul class="ptb_tabs">
		<li id="ptb_tab_files">files <span>(<?php echo ProfilerToolbar::$DATA_INC_FILES['total']['count']; ?>)</span>
		</li>
	</ul>
		...
		<table>
			...
			<?php foreach(ProfilerToolbar::$DATA_INC_FILES['data'] as $i => $file): ?>
				<tr>
					...
					<td>
						<?php
						if(strpos($file['name'], SYSPATH) === 0){
							$sep = $SYSPATH_sep;
						} elseif(strpos($file['name'], MODPATH) === 0){
							$sep = $MODPATH_sep;
						} elseif(strpos($file['name'], APPPATH) === 0){
							$sep = $APPPATH_sep;
						} elseif(strpos($file['name'], DOCROOT) === 0){
							$sep = $DOCROOT_sep;
						} else{
							$sep = 0;
						}
						?>
						<span style="color: #4a4a4a;"><?php echo substr($file['name'], 0,
								$sep); ?></span><?php echo substr($file['name'], $sep); ?>
					</td>
					...
					<td class="tCenter" style="color: #4a4a4a;">
						<?php echo date("Y.m.d", $file['lastModified']); ?>&nbsp;&nbsp;<?php echo date("H:i:s",
							$file['lastModified']); ?>
					</td>
				</tr>
			<?php endforeach; ?>
