...
class GeSHi{
					...
					for($i = 0; $i < $length; $i++){
						...
						if(($this->line_numbers != GESHI_NO_LINE_NUMBERS || count($this->highlight_extra_lines) > 0) && $char == "\n"){
							...
							$ESCAPE_CHAR_OPEN = false;
								...
								$this->language_data['QUOTEMARKS']) && ($STRING_OPEN == '') && $this->lexic_permissions['STRINGS']
						){
							...
							$stuff_to_parse = '';
						} else if($hq && substr($part, $i,
								strlen($hq)) == $hq && ($STRING_OPEN == '') && $this->lexic_permissions['STRINGS']
						){
								...
								if($STRING_OPEN == ''){
									...
									foreach($this->language_data['COMMENT_MULTI'] as $open => $close){
										...
										if($open == $test_str){
											...
											$rest_of_comment = GeSHi::hsc(substr($part, $i + $com_len,
												$close_pos - $i));
									...
									if(!$COMMENT_MATCHED){
										foreach($this->language_data['COMMENT_SINGLE'] as $comment_key => $comment_mark){
											...
											if($match){
												...
												$test_str .= GeSHi::hsc(substr($part, $i + $com_len,
													$close_pos - $i - $com_len));
												if($this->lexic_permissions['COMMENTS'][$comment_key]){
													$test_str .= "</span>";
												}
	...
	function add_url_to_keyword($keyword, $group, $start_or_end){
		...
		if(isset($this->language_data['URLS'][$group]) && $this->language_data['URLS'][$group] != '' && substr($keyword,
				0, 5) != '&lt;/'
		){
					...
					foreach($keywordset as $keyword){
						...
						if(false !== stristr($stuff_to_parse_pregquote, $keyword)){
							...
							if($this->language_data['CASE_SENSITIVE'][$k]){
								$stuff_to_parse = preg_replace("/([^a-zA-Z0-9\$_\|\#;>|^])($keyword)(?=[^a-zA-Z0-9_<\|%\-&])/e",
								...
								$stuff_to_parse = preg_replace("/([^a-zA-Z0-9\$_\|\#;>$hackage|^])($keyword)(?=[^a-zA-Z0-9_<\|%\-&])/ie",
									"'\\1' . $func2('\\2', '$k', 'BEGIN') . '<|$styles>' . $func('\\2') . '|>' . $func2('\\2', '$k', 'END')",
									$stuff_to_parse);
							}
		...
		if($this->lexic_permissions['METHODS'] && $this->language_data['OOLANG']){
			foreach($this->language_data['OBJECT_SPLITTERS'] as $key => $splitter){
				if(false !== stristr($stuff_to_parse, $splitter)){
					...
					$stuff_to_parse = preg_replace("/(" . preg_quote($this->language_data['OBJECT_SPLITTERS'][$key],
							1) . "[\s]*)([a-zA-Z\*\(][a-zA-Z0-9_\*]*)/", "\\1<|$attributes>\\2|>", $stuff_to_parse);
				}
			}
		}
	...
	function finalise($parsed_code){
		...
		if($this->enable_important_blocks && (strstr($parsed_code, GeSHi::hsc(GESHI_START_IMPORTANT)) === false)){
	...
	function footer(){
		...
		if($this->header_type == GESHI_HEADER_DIV){
			...
			return ($this->force_code_block ? '</div>' : '') . "$footer_content</div>";
		} else{
			if($this->line_numbers != GESHI_NO_LINE_NUMBERS){
				return "</ol>$footer_content</pre>";
			}
			return ($this->force_code_block ? '</div>' : '') . "$footer_content</pre>";
		}
	}
