 ...
 * @property integer $iduser_session
...
class UserSession extends Record
{
	...
	public function rules()
	{
		...
		return array(
			array('iduser, expired', 'numerical', 'integerOnly'=>true),
			array('token', 'length', 'max'=>100),
			array('ip', 'length', 'max'=>45),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('iduser_session, token, iduser, expired, ip', 'safe', 'on'=>'search'),
		);
	}
	...
	public function search()
	{
		...
		$criteria->compare('iduser_session',$this->iduser_session);
